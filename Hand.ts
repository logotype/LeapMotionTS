...
export class Hand
{
	 ...
	 * @see Finger
	 */
	public fingers:fingerModule.Finger[] = [];
	 ...
	 */
	public palmNormal:vectorModule.Vector3;

	/**
	 * The center position of the palm in millimeters from the Leap origin.
	 */
	public palmPosition:vectorModule.Vector3;

	/**
	 * The rate of change of the palm position in millimeters/second.
	 */
	public palmVelocity:vectorModule.Vector3;
	 ...
	 *
	 */
	public pointables:pointableModule.Pointable[] = [];

	/**
	 * The center of a sphere fit to the curvature of this hand.
	 */
	public sphereCenter:vectorModule.Vector3;
	/**
	 ...
	 * @see Tool
	 */
	public tools:toolModule.Tool[] =[];

	/**
	 * Rotation matrix.
	 */
	public rotation:matrixModule.Matrix;
	 ...
	 *
	 */
	public finger( id:number ):fingerModule.Finger
	{
		var returnValue:fingerModule.Finger = fingerModule.Finger.invalid();
		var i:number = 0;
	 ...
	 *
	 */
	public tool( id:number ):toolModule.Tool
	{
		var returnValue:toolModule.Tool = toolModule.Tool.invalid();
		var i:number = 0;
		var length:number = this.fingers.length;
	 ...
	 *
	 */
	public pointable( id:number ):pointableModule.Pointable
	{
		var returnValue:pointableModule.Pointable = pointableModule.Pointable.invalid();
		var i:number = 0;
	 ...
	 *
	 */
	public rotationAxis( sinceFrame:frameModule.Frame ):vectorModule.Vector3
	{
		var returnValue:vectorModule.Vector3;

		if ( sinceFrame.hand( this.id ) )
		{
			var vector:vectorModule.Vector3 = new vectorModule.Vector3( this.rotation.zBasis.y - sinceFrame.hand( this.id ).rotation.yBasis.z, this.rotation.xBasis.z - sinceFrame.hand( this.id ).rotation.zBasis.x, this.rotation.yBasis.x - sinceFrame.hand( this.id ).rotation.xBasis.y );
	 ...
	 *
	 */
	public rotationAngle( sinceFrame:frameModule.Frame, axis:vectorModule.Vector3 = null ):number
	{
		...
		if( !axis )
		{
			...
			{
				var rotationSinceFrameMatrix:matrixModule.Matrix = this.rotationMatrix( sinceFrame.hand( this.id ).frame );
		...
		else
		{
			if ( sinceFrame.hand( this.id ) && sinceFrame.hand( this.id ).frame )
			{
				var rotAxis:vectorModule.Vector3 = this.rotationAxis( sinceFrame.hand( this.id ).frame );
	 ...
	 *
	 */
	public rotationMatrix( sinceFrame:frameModule.Frame ):matrixModule.Matrix
	{
		var returnValue:matrixModule.Matrix;
	 ...
	 *
	 */
	public scaleFactor( sinceFrame:frameModule.Frame ):number
	{
	 ...
	 *
	 */
	public translation( sinceFrame:frameModule.Frame ):vectorModule.Vector3
	{
		var returnValue:vectorModule.Vector3;

		if ( sinceFrame.hand( this.id ) && sinceFrame.hand( this.id ).translationVector )
			returnValue = new vectorModule.Vector3( this.translationVector.x - sinceFrame.hand( this.id ).translationVector.x, this.translationVector.y - sinceFrame.hand( this.id ).translationVector.y, this.translationVector.z - sinceFrame.hand( this.id ).translationVector.z );
		else
			returnValue = new vectorModule.Vector3( 0, 0, 0 );
